...
from pa_core.wizard_schema import get_default_config, AnalysisMode, DefaultConfigView
...
class TestWizardConfigConsistency:
    ...
    def test_default_config_view_has_all_required_attributes(self):
        ...
        required_attributes = [
            'analysis_mode', 'n_simulations', 'n_months', 'total_fund_capital',
            'external_pa_capital', 'active_ext_capital', 'internal_pa_capital',
            'w_beta_h', 'w_alpha_h', 'theta_extpa', 'active_share',
            'mu_h', 'mu_e', 'mu_m', 'sigma_h', 'sigma_e', 'sigma_m',
            'rho_idx_h', 'rho_idx_e', 'rho_idx_m', 'rho_h_e', 'rho_h_m', 'rho_e_m',
            'risk_metrics'
        ]
        
        # Test for all analysis modes
        for mode in [AnalysisMode.RETURNS, AnalysisMode.CAPITAL, AnalysisMode.ALPHA_SHARES, AnalysisMode.VOL_MULT]:
            config = get_default_config(mode)
            
            for attr in required_attributes:
                assert hasattr(config, attr), f"DefaultConfigView missing required attribute '{attr}' for mode {mode.value}"
