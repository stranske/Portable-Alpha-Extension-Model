...
def test_load_index_returns_leaves_no_temp_files():
    ...
    suspicious_files = [f for f in new_files if f.startswith('tmp') and ('.csv' in f or '.yml' in f or '.yaml' in f)]
...
def test_temp_files_auto_cleanup_on_exception():
    ...
    assert len(suspicious_files) == 0, f"Temporary files left behind after exception: {suspicious_files}"
