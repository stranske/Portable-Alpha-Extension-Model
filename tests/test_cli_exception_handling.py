...
def test_attribution_exception_handling_attribute_error(caplog_debug):
    ...
    bad_returns = {"Base": "invalid_string", "ExternalPA": [1, 2, 3]}  # String has no .mean() method
    inputs_dict = {}
    ...
    try:
        ...
        logging.getLogger("pa_core.cli").warning(f"Attribution calculation failed due to data type issue: {e}")
...
def test_attribution_exception_handling_value_error(caplog_debug):
    ...
    try:
        ...
        logging.getLogger("pa_core.cli").warning(f"Attribution calculation failed due to configuration issue: {e}")
        inputs_dict["_attribution_df"] = pd.DataFrame(columns=["Agent", "Sub", "Return"])
...
def test_sensitivity_parameter_evaluation_value_error(caplog_debug):
    ...
    try:
        ...
        logging.getLogger("pa_core.cli").warning(f"Parameter evaluation failed for {pos_key} due to configuration: {e}")
    except (KeyError, TypeError) as e:
        failed_params.append(f"{pos_key}: Data type error: {str(e)}")
        skipped_params.append(pos_key)
        logging.getLogger("pa_core.cli").error(f"Parameter evaluation failed for {pos_key} due to data issue: {e}")
