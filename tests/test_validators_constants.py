...
class TestValidationConstants:
    ...
    def test_step_size_validation_uses_constant(self):
        ...
        results = validate_simulation_parameters(
            n_simulations=1000,
            step_sizes={'test_param': MIN_RECOMMENDED_STEP_SIZE}
        )
        
        # Should not trigger warning at exactly the threshold
        warnings = [r for r in results if r.severity == 'warning' and 'Very small step size' in r.message]
    ...
    def test_step_size_below_threshold_triggers_warning(self):
        ...
        results = validate_simulation_parameters(
            n_simulations=1000,
            step_sizes={'test_param': small_step}
        )
    ...
    def test_step_size_above_threshold_no_warning(self):
        ...
        results = validate_simulation_parameters(
            n_simulations=1000,
            step_sizes={'test_param': large_step}
        )
        
        # Should not trigger step size warning
        step_warnings = [r for r in results if r.severity == 'warning' and 'Very small step size' in r.message]
    ...
    def test_constant_consistency(self):
        ...
        results = validate_simulation_parameters(
            n_simulations=1000,
            step_sizes={'param': MIN_RECOMMENDED_STEP_SIZE - 0.001}
        )
