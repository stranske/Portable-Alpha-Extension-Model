...
from pa_core.config import get_field_mappings, ModelConfig
...
def test_get_field_mappings():
    ...
    for alias, field_name in expected_core_mappings.items():
        ...
        assert mappings[alias] == field_name, f"Wrong mapping for '{alias}': expected '{field_name}', got '{mappings[alias]}'"
...
def test_field_mappings_consistency():
    ...
    for alias, field_name in mappings.items():
        assert field_name in model_fields, f"Field '{field_name}' mapped from alias '{alias}' does not exist in ModelConfig"
