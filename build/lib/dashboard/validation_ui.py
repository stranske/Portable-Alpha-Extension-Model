...
def create_validation_sidebar() -> Dict[str, Any]:
    ...
    # Validation controls
    validate_on_change = st.sidebar.checkbox(
        "Real-time validation",
        value=True,
        help="Validate parameters as you change them",
    )
    ...
    show_warnings = st.sidebar.checkbox(
        "Show warnings", value=True, help="Display validation warnings"
    )
...
def display_psd_projection_info(psd_info: Dict[str, Any]) -> None:
    ...
    with st.expander("Projection Details", expanded=False):
        st.write(
            "**Original min eigenvalue:**",
            f"{psd_info.get('original_min_eigenvalue', 0):.6f}",
        )
        st.write(
            "**Projected min eigenvalue:**",
            f"{psd_info.get('projected_min_eigenvalue', 0):.6f}",
        )
...
def create_margin_buffer_display(
    margin_requirement: float, available_buffer: float, total_capital: float = 1000.0
) -> None:
