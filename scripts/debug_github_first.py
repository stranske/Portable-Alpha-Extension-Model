...
def main():
    ...
    parser.add_argument(
        "--branch", help="Specific branch to debug (auto-detected if not provided)"
    )
    parser.add_argument(
        "--skip-push", action="store_true", help="Don't push fixes automatically"
    )
    parser.add_argument(
        "--force-local", action="store_true", help="Force full local debugging"
    )
