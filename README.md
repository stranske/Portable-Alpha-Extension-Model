...
- Regimes apply in parameter sweeps: when `regimes` and `regime_transition` are configured, every sweep grid point draws regime-switching returns.
...
If you set `backend: cupy` in a config file or pass `--backend cupy`, the run fails with a
