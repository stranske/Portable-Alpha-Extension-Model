...
1. **Create and validate a scenario** – draft a YAML configuration and verify the schema with `pa validate`. A minimal example is provided in `templates/scenario_example.yaml`:
   ...
   > - `correlations` must include one pair per unique index/asset combination; missing, duplicate, or extra pairs are rejected.
...
To run a multi-scenario sweep, create a YAML file with your sweep parameters
and specify the appropriate mode:
...
The primary configuration file is `config/params_template.yml`. Copy and modify
it to create sweep configurations for different analysis modes. Replace the index
file with your own returns series and set `--output` to avoid overwriting previous runs.
...
appropriate `analysis_mode`. See the Parameter Sweep Engine section for the
key parameters used by each mode.
...
1. **Create a capital sweep config** by copying `params_template.yml`:
   ...
   ```
   Edit the file to set `analysis_mode: capital` and add capital ranges:
   ```yaml
   analysis_mode: capital
   external_pa_capital: [80, 100, 120]
   active_ext_capital: [20, 40, 60]
   internal_pa_capital: [100]
   ```
...
3. **Convenience fields still work** – you can keep the legacy capital/share fields without defining `agents`:
   ```yaml
   ...
   active_share: 0.5
   ```
4. **Extend the configuration** – add your agent entry to the `agents` list in YAML. Each entry uses `{name, capital, beta_share, alpha_share, extra}`:
   ```yaml
   agents:
     - name: MyAgent
       capital: 25.0
       ...
       extra:
         my_agent_param: 0.5
   ```
5. (Optional) Keep the convenience fields for the core sleeves and append your custom agent in mixed mode:
   ```yaml
   external_pa_capital: 100.0
   active_ext_capital: 50.0
   internal_pa_capital: 150.0
...
6. Run the CLI again and the sleeve will automatically appear in the outputs.
...
7. **Run a parameter sweep** – create a CSV file `custom_agent_sweep.csv`
...
8. **Visualise the results** – load `MyAgentSweep.xlsx` in the dashboard
