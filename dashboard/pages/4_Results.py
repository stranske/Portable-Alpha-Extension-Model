...
def main() -> None:
    ...
    if te_column is not None:
        col1.metric("Tracking Error", f"{summary[te_column].mean():.2%}", help=tooltip("monthly_TE"))
    if SUMMARY_CVAR_COLUMN in summary:
        col2.metric("monthly_CVaR", f"{summary[SUMMARY_CVAR_COLUMN].mean():.2%}", help=tooltip("monthly_CVaR"))
